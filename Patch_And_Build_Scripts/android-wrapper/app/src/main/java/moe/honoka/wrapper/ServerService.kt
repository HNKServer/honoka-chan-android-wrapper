...
class ServerService : Service() {
    ...
    private fun requiredServerFiles(serverDir: File): List<File> = listOf(
        ...
        File(serverDir, "assets/data.example.db"),
        File(serverDir, "privatekey.pem"),
        File(serverDir, "publickey.pem"),
        File(serverDir, "certificate.crt"),
        File(serverDir, "server.crt")
    )
    ...
    private fun prepareServerBase(serverDir: File) {
        ...
        if (marker.exists() && hasRequiredServerFiles(serverDir)) {
            return
        }
        ...
        if (marker.exists()) {
            Log.w(
                "HonokaServer",
                "base marker exists but required files are missing; re-extracting server-base.zip"
            )
            marker.delete()
        }
                ...
                ZipInputStream(input).use { zis ->
                    while (true) {
                        ...
                        if (entry.isDirectory || normalizedName.endsWith("/")) {
                            ...
                            if (normalizedName == "config.json" && outFile.exists()) {
                                Log.i(
                                    "HonokaServer",
