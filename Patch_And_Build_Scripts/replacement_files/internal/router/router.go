...
func SifRouter(r *gin.Engine) {
	...
	// Android wrapper control endpoints. These are deliberately loopback-only.
	android := r.Group("/__android")
	android.Use(func(ctx *gin.Context) {
		if !isLoopbackRequest(ctx) {
			ctx.AbortWithStatusJSON(http.StatusForbidden, gin.H{
				"ok":    false,
				"error": "Android wrapper control endpoint is localhost-only",
			})
			return
		}
		ctx.Next()
	})
	android.GET("/health", func(ctx *gin.Context) {
		ctx.JSON(http.StatusOK, gin.H{
			"ok":          true,
			"app_name":    config.Conf.AppName,
			"version":     config.PackageVersion,
			"listen_port": config.Conf.Settings.ListenPort,
		})
	})
	android.POST("/config/reload", func(ctx *gin.Context) {
		if err := config.ReloadStrict("./config.json"); err != nil {
...
func isLoopbackRequest(ctx *gin.Context) bool {
